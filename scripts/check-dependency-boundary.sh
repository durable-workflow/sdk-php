...
if grep -Eqi '"name"[[:space:]]*:[[:space:]]*"(laravel/|illuminate/|durable-workflow/workflow")' <<<"$manifest"; then
  echo "Production dependency graph crosses the framework-neutral SDK boundary." >&2
  exit 1
fi

if grep -Eqi '"(laravel/|illuminate/|durable-workflow/workflow)"[[:space:]]*:' composer.json; then
