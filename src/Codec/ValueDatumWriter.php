 ...
 *
 * The private binding schema represents a map as an array of key/value records,
 * which has the same Avro binary layout as a map while keeping numeric-looking
 * keys in string values. Apache's datum writer performs validation and binary
 * encoding.
 */
final class ValueDatumWriter
{
    ...
    {
        return self::$bindingSchema ??= AvroPayloadCodec::parseSchema(self::BINDING_SCHEMA_JSON);
    }
}
